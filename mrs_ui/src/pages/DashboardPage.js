...
const DashboardPage = () => {
	...
	const fetchData = (request, chartType) => {
		...
		axios
			...
			.then((result) => {
				console.log("energyAndCapacityDecayData", result.data.records[0]);
				...
				ref.current.getEchartsInstance().setOption({
					...
					dataset: energyAndCapacityDecayData,
					series: _createChartDataSeries(
						energyAndCapacityDecayData, // replace with actual data
						xAxis.mapToId,
						yAxis.mapToId
					),
					xAxis: {
					...
					legend: _createChartLegend(energyAndCapacityDecayData),
					toolbox: {
						feature: {
							myTool: {
								show: true,
	...
	const _createChartDataSeries = (data, xAxis, yAxis) => {
		let x = [];
		data.forEach((d) => {
			x.push({
				type: "line",
