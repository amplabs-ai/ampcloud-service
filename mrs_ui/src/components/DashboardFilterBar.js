...
const DashboardFilterBar = (props) => {
	...
	useEffect(() => {
		axios.get("http://localhost:4000/cells/all/meta").then((response) => {
	...
	const handleCellDelete = (record) => {
		...
		axios
			.delete(`http://localhost:4000/cells/${record.cell_id}`)
			.then(() => {
	...
	const downloadCycleData = (k) => {
		...
		axios
			.get(`http://localhost:4000/download/cells/cycle_data/${k}`)
			.then(({ data }) => {
	...
	const downloadTimeSeriesData = (k) => {
		...
		axios
			.get(`http://localhost:4000/download/cells/cycle_timeseries/${k}`)
			.then(({ data }) => {
