...
function App() {
  ...
  const toEdit=(arg)=>{
    navigate('/edit',{state:{cell_id: arg[0], anode: arg[1], cathode: arg[2], source: arg[3], form_factor: arg[4], ah: arg[5] }});
  }
  const toCreate=()=>{
    navigate('/create');
  }
    ...
    columns: [
      ...
      },
      {
      ...
      },
       {
        Header: "Edit/Delete",
        Cell: ({ cell }) => ( <>
        <button className='CRUD' onClick={()=>{toEdit([cell.row.values.cell_id, cell.row.values.anode, cell.row.values.cathode, cell.row.values.source, cell.row.values.form_factor, cell.row.values.ah ])}}>
          {"Edit"}
        </button>
