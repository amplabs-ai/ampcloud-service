import json
import logging
from app.archive_constants import LINKEDIN_CLIENT_ID, LINKEDIN_CLIENT_SECRET, LINKEDIN_REDIRECT_URI_DASH_ABUSE, \
    LINKEDIN_REDIRECT_URI_DASH_CYCLE
from app.response import Response
from app.services.dashboard_share_service import dashboard_share_add_service, dashboard_share_update_service, \
    dashboard_share_validate_id_service
...
def dashboard_share_linkedin():
    try:
        ...
        url = "https://www.linkedin.com/oauth/v2/accessToken?grant_type=authorization_code&code=" + authCode + \
              "&redirect_uri=" + redirectUri + "&client_id=" + LINKEDIN_CLIENT_ID + "&client_secret=" + \
        ...
        json_data = json.loads(response.text)
        uploadUrl = json_data.get('value').get('uploadMechanism').get(
