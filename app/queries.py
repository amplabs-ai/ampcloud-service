...
FROM cycle_stats
where cell_id IN {cell_id} and cycle_coulombic_efficiency<1.004 and (email in ('{email}', 'info@batteryarchive.org', 'data.matr.io@tri.global') or email in (select email from user_plan where plan_type = 'COMMUNITY'))) as r
...
FROM cycle_stats
where cell_id IN {cell_id} and cycle_coulombic_efficiency<1.1 and (email in ('{email}', 'info@batteryarchive.org', 'data.matr.io@tri.global') or email in (select email from user_plan where plan_type = 'COMMUNITY'))) as r
...
where
    ...
    (MOD(cycle_index,{step})=0 or cycle_index = 1 or cycle_index = ( SELECT MAX(cycle_index) FROM cycle_stats WHERE cell_id IN {cell_id} and (email in ('{email}', 'info@batteryarchive.org', 'data.matr.io@tri.global') or email in (select email from user_plan where plan_type = 'COMMUNITY')))) and
    (email in ('{email}', 'info@batteryarchive.org', 'data.matr.io@tri.global') or email in (select email from user_plan where plan_type = 'COMMUNITY'))
...
FROM
  (SELECT cycle_timeseries.cell_id,
   ...
   FROM cycle_timeseries
   WHERE cell_id IN {cell_id} and (email in ('{email}', 'info@batteryarchive.org', 'data.matr.io@tri.global') or email in (select email from user_plan where plan_type = 'COMMUNITY'))
     ) AS r
...
FROM
  (SELECT abuse_timeseries.cell_id,
   ...
   WHERE cell_id IN {cell_id} and (email in ('{email}', 'info@batteryarchive.org', 'data.matr.io@tri.global') or email in (select email from user_plan where plan_type = 'COMMUNITY'))) AS r
...
FROM
    cycle_timeseries
WHERE cell_id IN ({cell_ids}) and (email in ('{email}', 'info@batteryarchive.org', 'data.matr.io@tri.global') or email in (select email from user_plan where plan_type = 'COMMUNITY'))
